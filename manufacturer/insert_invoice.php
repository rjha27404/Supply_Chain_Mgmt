<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		if(!isset($_POST['distributor'])) {
			...
			$query_selectInvoiceId = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='supply_chain_mgmt' AND TABLE_NAME='invoice'";
			...
			if(mysqli_query($con,$queryInsertInvoice)) {
				...
				while($row_selectOrderItems = mysqli_fetch_array($result_selectOrderItems)) {
					...
					$query = mysqli_query($con,"SELECT invoice_id FROM invoice WHERE order_id='$order_id' AND retailer_id='$retailer_id' AND dist_id='$dist_id'");
