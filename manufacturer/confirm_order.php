<?php
	...
	session_start();
		if(isset($_SESSION['manufacturer_login'])) {
			...
			$queryAvailQuantity = "SELECT products.pro_id AS pro_id,products.quantity AS quantity FROM order_items,products WHERE products.pro_id=order_items.pro_id AND order_items.order_id='$id' AND products.quantity>0";
			...
			while($rowAvailQuantity = mysqli_fetch_array($resultAvailQuantity)){
				array_push($availProId,$rowAvailQuantity['pro_id']);
				echo $rowAvailQuantity['quantity'];
				array_push($availQuantity,$rowAvailQuantity['quantity']);
				
			}
			$orderProId =  array();
			$orderQuantity = array();
			while($rowOrderQuantity = mysqli_fetch_array($resultOrderQuantity)){
				array_push($orderProId,$rowOrderQuantity['p']);
				array_push($orderQuantity,$rowOrderQuantity['q']);
			}
